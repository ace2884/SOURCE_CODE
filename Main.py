from tkinter import messagebox, simpledialog, filedialog, Scrollbar
from tkinter import *
import tkinter
...
filename = None
train = None
svm_acc = nn_acc = svmga_acc = annga_acc = 0.0
X_train = X_test = y_train = y_test = None
svmga_classifier = None
nnga_classifier = None
svm_time = svmga_time = nn_time = nnga_time = 0.0
...
def generateModel():
    ...
    text.delete('1.0', END)
    if not filename:
        messagebox.showwarning('No file','Please upload a dataset file first')
        return
    try:
...
def prediction(X_test, cls):  #prediction done here
    try:
        ...
        # fallback if wrapper exposes estimator_
        y_pred = getattr(cls, 'estimator_', cls).predict(X_test)
...
def runNN():
    ...
    try:
        ...
        messagebox.showerror('NN error', str(e))
        return
...
def runNNGenetic():
    ...
    features = cols - 1
    X = train_df.values[:, 0:100]
    Y = train_df.values[:, features]
    ...
    try:
        ...
        messagebox.showerror('NN Genetic error', str(e))
...
def timeGraph():
    try:
        ...
        messagebox.showerror('Time graph error', str(e))

    
font = ('times', 16, 'bold')
title = Label(main, text='PAD: Towards Principled Adversarial Malware Detection Against Evasion Attacks')
